...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[BUYER-OTP] Generated OTP for user ${userId}, amount: $${amount}`);

    // Delete any existing OTPs for this buyer
    await serviceClient
...
<body style="margin: 0; padding: 40px 20px; background-color: #f8f9fa; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif;">
  <div style="max-width: 560px; margin: 0 auto;">
    ...
    <div style="text-align: center; padding: 32px 20px;">
      ...
      console.log(`[BUYER-OTP] Sending email to ${userEmail} via Resend...`);
      ...
      if (!emailRes.ok) {
        ...
        console.error("[BUYER-OTP] Email send failed:", errorText);
      ...
      console.log(`[BUYER-OTP] Email sent successfully`);
    } else {
      console.error("[BUYER-OTP] Missing RESEND_API_KEY or user email");
    ...
    console.log(`[BUYER-OTP] Success - OTP sent to ${userEmail}`);
    return new Response(
      JSON.stringify({ 
        success: true, 
