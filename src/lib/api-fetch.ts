...
export const bffApi = {
  ...
  getPromptsData: () => apiFetch<{
    ...
    favorites: string[];
    isPro: boolean;
    _meta: { fetchedAt: string; userId: string; totalPrompts: number };
  }>('bff-prompts-data'),

  // Marketplace data (AI accounts + seller products)
  getMarketplace: () => apiFetch<{
    aiAccounts: any[];
    sellerProducts: any[];
    categories: { all: any[]; aiAccounts: any[]; products: any[] };
    wallet: { balance: number };
    purchases: { aiAccounts: any[]; sellerOrders: any[] };
    _meta: { fetchedAt: string; userId: string };
  }>('bff-marketplace'),

  // Billing data
  getBillingData: () => apiFetch<{
    ...
    paymentMethods: any[];
    profile: { isPro: boolean; email: string };
    _meta: { fetchedAt: string; userId: string };
  }>('bff-billing-data'),

  // Chat data
  getChatData: () => apiFetch<{
    support: { messages: any[]; unreadCount: number };
    sellerChats: { conversations: any[]; unreadCount: number };
    _meta: { fetchedAt: string; userId: string };
  }>('bff-chat-data'),

  // Public endpoints (no auth required)
  getPublicStore: (slug: string) => publicFetch<{
    seller: any;
    products: any[];
    categories: any[];
    _meta: { fetchedAt: string; totalProducts: number };
  }>(`bff-store-public?slug=${encodeURIComponent(slug)}`),

  getPublicProduct: (productId: string) => publicFetch<{
    product: any;
    reviews: any[];
    avgRating: number;
    reviewCount: number;
    _meta: { fetchedAt: string };
  }>(`bff-store-public?productId=${encodeURIComponent(productId)}`),
};
