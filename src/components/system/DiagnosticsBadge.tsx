 ...
 * Shows auth status, storage access, key count, and session health.
 */
export default function DiagnosticsBadge() {
  ...
  // Compute diagnostics when panel opens
  const storageAccess = useMemo(() => healthMonitor.testLocalStorageAccess(), [open]);
  const keyCount = useMemo(() => healthMonitor.countAuthStorageKeys(), [open]);
        ...
        <div className="mt-2 w-[320px] rounded-xl border border-border bg-background/95 backdrop-blur p-3 shadow-lg">
          ...
          <div className="mt-2 space-y-1.5 text-xs">
            ...
            <div className="flex justify-between gap-2">
              ...
              <span className={`font-medium ${storageAccess ? 'text-primary' : 'text-destructive'}`}>
                {storageAccess ? 'ok' : 'BLOCKED'}
              </span>
            </div>
            ...
            <div className="flex justify-between gap-2">
              ...
              <span className={`font-medium ${keyCount > 0 ? 'text-primary' : 'text-destructive'}`}>
                {keyCount} {keyCount === 0 ? '(missing!)' : ''}
              </span>
            </div>
...
{healthMonitor.getLogs().slice(-15).map((log) => {
