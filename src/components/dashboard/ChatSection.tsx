...
const ChatSection = () => {
  ...
  useEffect(() => {
    ...
    const fetchTickets = async () => {
      ...
      if (ticketsData) {
        ...
        if (!activeTicketId && ticketsWithUnread.length > 0) {
          setActiveTicketId(ticketsWithUnread[0].id);
        }
      }
      setLoading(false);
    };
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`support-msgs-${activeTicketId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'support_messages', filter: `ticket_id=eq.${activeTicketId}` }, () => {
        fetchMessages();
      })
      .subscribe();
  ...
  const handleSend = async () => {
    if (!messageText.trim() || !user || !activeTicketId || sending) return;
    setSending(true);
  ...
  const handleCreateTicket = async () => {
    ...
    if (error) {
      ...
      const newTicket = { ...(data as any), unreadCount: 0 };
      ...
      <aside className="w-[320px] border-r flex-col hidden lg:flex flex-shrink-0" style={{ borderColor: '#e2e8f0', background: '#fff' }}>
        <div style={{ padding: '24px 20px 16px' }}>
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              onClick={() => setCreatingTicket(!creatingTicket)}
              className="flex items-center gap-1 cursor-pointer px-2 py-1 rounded-md hover:bg-[#f1f5f9]"
          ...
          {creatingTicket && (
            <div className="mb-3 flex gap-2">
              <input
                ...
                className="flex-1 h-9 rounded-lg px-3 outline-none"
                style={{ border: '1px solid #e2e8f0', fontSize: '13px' }}
              />
              <button onClick={handleCreateTicket} className="h-9 px-3 rounded-lg text-white" style={{ background: '#2563eb', fontSize: '12px' }}>
                Create
              </button>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6">
            ...
            <div className="mb-5">
              ...
              <div className="w-full px-3 py-[10px] rounded-lg flex items-center justify-between" style={{ border: '1px solid #e2e8f0', fontSize: '13px' }}>
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 rounded-full flex items-center justify-center text-white" style={{ background: '#94a3b8', fontSize: '9px' }}>S</div>
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2 flex-wrap">
                {(activeTicket.tags || ['Question']).map((tag) => (
            ...
            <div className="mb-5">
              ...
              <div className="rounded-lg p-4" style={{ background: '#f8fafc' }}>
                {[
                  ...
                  { key: 'Status', val: activeTicket.status },
                  { key: 'Date submitted', val: format(new Date(activeTicket.created_at), 'dd MMM yyyy, HH:mm') },
                ].map((attr, i, arr) => (
