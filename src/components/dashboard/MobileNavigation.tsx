...
import { Sparkles, ShoppingBag, Wallet, MessageSquare, User, Bell } from 'lucide-react';
...
const MobileNavigation = () => {
  ...
  return (
    <>
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 bg-white z-50 border-b border-gray-100 safe-area-top">
        <div className="flex items-center justify-between px-4 h-14">
          {/* Logo */}
          <Link to="/dashboard" className="flex items-center">
            <img src={theLogo} alt="Uptoza" className="h-7 w-auto" />
          </Link>
          
          {/* Notification Bell with Dropdown */}
          <DropdownMenu open={showDropdown} onOpenChange={setShowDropdown}>
            <DropdownMenuTrigger asChild>
              <button className="relative p-2.5 hover:bg-gray-100 active:scale-95 rounded-full transition-all mobile-touch-target">
            ...
            <DropdownMenuContent 
              align="end" 
              className="w-80 bg-white border border-gray-200 shadow-xl z-[100] rounded-xl p-0 overflow-hidden"
            >
              {/* Header */}
