...
import { GumroadWelcomeIcon, GumroadProfileIcon, GumroadProductIcon, GumroadFollowerIcon, GumroadSaleIcon, GumroadPayoutIcon, GumroadEmailBlastIcon, GumroadSmallBetsIcon, GumroadInfoIcon, GumroadCollapseIcon } from './GumroadIcons';
...
export const GettingStartedSection = ({
  ...
  return <div className="mb-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => setIsCollapsed(!isCollapsed)} className="flex items-center gap-1 text-sm underline hover:no-underline">
          <span>Show less</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        const CardContent = <div className="relative bg-white border rounded p-4 text-center hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-shadow cursor-pointer">
              {/* Completion Badge */}
              {item.isCompleted ? <span className="absolute top-2 right-2 w-5 h-5 bg-teal-600 rounded-full flex items-center justify-center">
                  <Check className="w-3 h-3 text-white" strokeWidth={3} />
                </span> : <span className="absolute top-2 right-2 w-5 h-5 bg-neutral-800 rounded-full border-2 border-neutral-800" />}
...
export const ActivityEmptyState = () => {
  return <div className="bg-white border border-dashed rounded p-6 text-center">
...
export const EmptyProductState = () => {
  return <div className="bg-white border border-dashed rounded p-6 text-center">
      <img src="https://assets.gumroad.com/packs/static/20f9d3e0a6869c1b28a1.png" alt="Empty state illustration" className="w-full max-w-md mx-auto mb-3" />
      <h2 className="text-xl font-semibold text-slate-900 mb-3">
        We're here to help you get paid for your work.
      </h2>
      <Link to="/seller/products/new" className="inline-flex items-center bg-[#FF90E8] border border-black px-4 py-3 rounded hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-shadow font-medium">
        Create your first product
      </Link>
      <div className="mt-3">
        <Link to="/dashboard/marketplace" className="underline text-sm text-slate-600 hover:no-underline">
