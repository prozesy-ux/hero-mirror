import { Users, Star, Package, RefreshCw } from 'lucide-react';
import { CardLayoutProps, getBorderRadiusClass } from '../card-types';
import { CARD_STYLE_PRESETS } from '../card-styles';
import { OptimizedImage } from '@/components/ui/optimized-image';

const MembershipCard = ({ product, settings, sellerName, onClick, onBuy, purchasing }: CardLayoutProps) => {
    ...
    <div onClick={onClick} className={`group overflow-hidden cursor-pointer transition-all duration-300 ${br} ${style.container} ${style.hoverEffect}`}>
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs text-slate-500">{product.member_count || 0} members</span>
        </div>
        <div className="flex items-center justify-between mb-3">
          <span className={`text-lg ${style.price}`} style={{ color: settings.accentColor }}>${product.price}<span className="text-xs text-slate-400">/mo</span></span>
        ...
        <button onClick={(e) => { e.stopPropagation(); onBuy?.(); }} disabled={purchasing}
          ...
          {purchasing ? '...' : (settings.buttonText === 'Buy' ? 'Join' : settings.buttonText)}
        </button>
      </div>
    </div>
  );
