import { Monitor, Apple, Globe, Star, Package } from 'lucide-react';
import { CardLayoutProps, getBorderRadiusClass } from '../card-types';
import { CARD_STYLE_PRESETS } from '../card-styles';
import { OptimizedImage } from '@/components/ui/optimized-image';

const SoftwareCard = ({ product, settings, sellerName, onClick, onBuy, purchasing }: CardLayoutProps) => {
    ...
    <div onClick={onClick} className={`group overflow-hidden cursor-pointer transition-all duration-300 ${br} ${style.container} ${style.hoverEffect}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute bottom-2 left-2 flex gap-1">
          <div className="w-6 h-6 rounded bg-white/90 flex items-center justify-center shadow"><Monitor size={12} className="text-slate-600" /></div>
          <div className="w-6 h-6 rounded bg-white/90 flex items-center justify-center shadow"><Apple size={12} className="text-slate-600" /></div>
          <div className="w-6 h-6 rounded bg-white/90 flex items-center justify-center shadow"><Globe size={12} className="text-slate-600" /></div>
        </div>
      </div>
      <div className="p-3">
