import { Palette, Clock, Star, Package } from 'lucide-react';
import { CardLayoutProps, getBorderRadiusClass } from '../card-types';
import { CARD_STYLE_PRESETS } from '../card-styles';
import { OptimizedImage } from '@/components/ui/optimized-image';

const CommissionCard = ({ product, settings, sellerName, onClick, onBuy, purchasing }: CardLayoutProps) => {
  ...
  const deposit = Math.round(product.price * 0.5);
    ...
    <div onClick={onClick} className={`group overflow-hidden cursor-pointer transition-all duration-300 ${br} ${style.container} ${style.hoverEffect}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className={`absolute top-2 left-2 px-2 py-1 text-[10px] font-bold ${style.badge} ${br} flex items-center gap-1`}>
          <Palette size={10} /> Commission
        </div>
      </div>
      <div className="p-3">
        <h3 className={`text-sm leading-tight line-clamp-2 mb-1.5 ${style.title}`}>{product.name}</h3>
        {settings.showSellerName && sellerName && <p className="text-xs text-slate-500 mb-1 truncate">by {sellerName}</p>}
        <div className="flex items-center gap-3 mb-2 text-xs text-slate-500">
          <span>50% deposit: ${deposit}</span>
          <span className="flex items-center gap-1"><Clock size={10} />{product.delivery_time || '7 days'}</span>
        </div>
        ...
        <button onClick={(e) => { e.stopPropagation(); onBuy?.(); }} disabled={purchasing}
          ...
          {purchasing ? '...' : (settings.buttonText === 'Buy' ? 'Request' : settings.buttonText)}
        </button>
      </div>
    </div>
  );
