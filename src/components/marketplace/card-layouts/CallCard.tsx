import { Calendar, Clock, Star, Package } from 'lucide-react';
import { CardLayoutProps, getBorderRadiusClass } from '../card-types';
import { CARD_STYLE_PRESETS } from '../card-styles';
import { OptimizedImage } from '@/components/ui/optimized-image';

const CallCard = ({ product, settings, sellerName, onClick, onBuy, purchasing }: CardLayoutProps) => {
    ...
    <div onClick={onClick} className={`group overflow-hidden cursor-pointer transition-all duration-300 ${br} ${style.container} ${style.hoverEffect}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute top-2 left-2 flex gap-1">
          <div className={`px-2 py-1 text-[10px] font-bold ${style.badge} ${br} flex items-center gap-1`}>
            <Calendar size={10} /> Schedule
          </div>
        </div>
        {product.duration && (
          <div className={`absolute top-2 right-2 px-2 py-1 text-[10px] font-bold ${style.badge} ${br} flex items-center gap-1`}>
      ...
      <div className="p-3">
        ...
        <button onClick={(e) => { e.stopPropagation(); onBuy?.(); }} disabled={purchasing}
          ...
          {purchasing ? '...' : (settings.buttonText === 'Buy' ? 'Schedule' : settings.buttonText)}
        </button>
      </div>
    </div>
  );
