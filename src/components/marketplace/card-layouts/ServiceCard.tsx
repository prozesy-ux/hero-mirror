...
const ServiceCard = ({ product, settings, sellerName, onClick, onBuy, purchasing }: CardLayoutProps) => {
  ...
  return (
    <div onClick={onClick} className={`group overflow-hidden cursor-pointer transition-all duration-300 ${br} ${style.container} ${style.hoverEffect}`}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {/* Availability dot */}
        <div className="absolute top-2 right-2 flex items-center gap-1.5 px-2 py-1 bg-white/90 rounded-full shadow-sm">
          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          <span className="text-[10px] font-medium text-slate-700">Available</span>
        </div>
      </div>
      <div className="p-3">
        ...
        <div className="flex items-center gap-1 text-xs text-slate-500 mb-2">
          <Clock size={11} /> <span>{product.response_time || 'Fast response'}</span>
        </div>
        ...
        <button onClick={(e) => { e.stopPropagation(); onBuy?.(); }} disabled={purchasing}
          ...
          {purchasing ? '...' : (settings.buttonText === 'Buy' ? 'Book Now' : settings.buttonText)}
        </button>
      </div>
    </div>
  );
