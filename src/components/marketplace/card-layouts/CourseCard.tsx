import { BookOpen, Star, Package } from 'lucide-react';
import { CardLayoutProps, getBorderRadiusClass } from '../card-types';
import { CARD_STYLE_PRESETS } from '../card-styles';
import { OptimizedImage } from '@/components/ui/optimized-image';

const CourseCard = ({ product, settings, sellerName, onClick, onBuy, purchasing }: CardLayoutProps) => {
  ...
  return (
    <div onClick={onClick} className={`group overflow-hidden cursor-pointer transition-all duration-300 ${br} ${style.container} ${style.hoverEffect}`}>
      <div className="relative aspect-video overflow-hidden">
        <OptimizedImage src={product.icon_url} alt={product.name} aspectRatio="video" className="w-full h-full transition-transform duration-500 group-hover:scale-105" fallbackIcon={<Package className="h-12 w-12 text-slate-300" />} />
        ...
        {settings.showBadge && (
          <div className={`absolute top-2 left-2 px-2 py-1 text-[10px] font-bold ${style.badge} ${br} flex items-center gap-1`}>
            <BookOpen size={10} /> {product.lesson_count || 0} lessons
          </div>
        )}
      </div>
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {settings.showRating && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-slate-500">{product.rating?.toFixed(1) || '0'}</span>
            </div>
          )}
        </div>
        <button onClick={(e) => { e.stopPropagation(); onBuy?.(); }} disabled={purchasing}
          ...
          {purchasing ? '...' : (settings.buttonText === 'Buy' ? 'Start Learning' : settings.buttonText)}
        </button>
      </div>
    </div>
  );
