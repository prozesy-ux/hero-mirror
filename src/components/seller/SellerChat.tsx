...
const SellerChat = () => {
  const { profile: sellerProfile } = useSellerContext();
  ...
  const fetchUnreadCounts = useCallback(async () => {
    if (!sellerId || tickets.length === 0) return;
    const { data } = await supabase
      .from('seller_support_messages')
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'seller_support_messages', filter: `seller_id=eq.${sellerId}` }, () => {
        fetchMessages();
        fetchUnreadCounts();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'support_tickets', filter: `seller_id=eq.${sellerId}` }, () => {
        fetchTickets();
      })
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [sellerId, fetchMessages, fetchTickets, fetchUnreadCounts]);

  const handleSend = async () => {
    if (!messageText.trim() || !sellerId || !activeTicketId) return;
    setSending(true);
  ...
  const handleCreateTicket = async () => {
    if (!newSubject.trim() || !newFirstMessage.trim() || !sellerId) return;
    setCreatingTicket(true);

    // Get the user_id for the seller to satisfy the RLS
    ...
    const { data: ticket, error } = await supabase
      .from('support_tickets')
      .insert({ seller_id: sellerId, subject: newSubject.trim(), user_id: sellerData?.user_id } as any)
      .select()
      .single();
    if (error || !ticket) {
      ...
      <aside className="w-[320px] border-r flex-col hidden lg:flex flex-shrink-0" style={{ borderColor: '#e2e8f0', background: '#fff' }}>
        ...
        <div className="flex-1 overflow-y-auto px-3">
          {filteredTickets.length === 0 ? (
            <div className="text-center py-8" style={{ color: '#94a3b8', fontSize: '13px' }}>No tickets yet</div>
          ...
          <>
            <header className="flex items-center justify-between flex-shrink-0 px-8" style={{ height: '72px', borderBottom: '1px solid #e2e8f0' }}>
              <div className="flex items-center gap-4">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer lg:hidden" style={{ border: '1px solid #e2e8f0' }} onClick={() => setActiveTicketId(null)}>
                  <ChevronLeft size={18} />
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <Star size={16} onClick={toggleStar} className="cursor-pointer"
                ...
                return (
                  <div key={msg.id} className={cn("flex gap-4", isAdmin ? "self-end flex-row-reverse" : "")} style={{ maxWidth: '80%' }}>
                    ...
                    <div className={cn("flex flex-col gap-[6px]", isAdmin ? "items-end" : "")}>
                      {!isAdmin && <div className="font-semibold" style={{ fontSize: '13px' }}>{sellerProfile?.store_name || 'You'}</div>}
                      {isAdmin && (
                        <div className="flex items-center gap-[6px] font-medium" style={{ fontSize: '12px', color: '#7c3aed' }}>
                          <Sparkles size={12} /> Support Agent
                        </div>
                      )}
                      <div className="px-[18px] py-[14px] rounded-xl" style={{
                        fontSize: '14px', lineHeight: '1.6', boxShadow: '0 1px 2px rgba(0,0,0,0.05)',
                        ...(isAdmin ? { background: '#eff6ff', color: '#1e3a8a', borderTopRightRadius: '4px' } : { background: '#f1f5f9', borderTopLeftRadius: '4px' }),
                      }}>
                        {msg.message}
                      </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6">
            <div className="mb-5">
              ...
              <div className="w-full px-3 py-[10px] rounded-lg flex items-center gap-2" style={{ border: '1px solid #e2e8f0', fontSize: '13px' }}>
                <User size={14} style={{ color: '#64748b' }} /> {activeTicket.assigned_to || 'Unassigned'}
              </div>
            </div>
            <div className="mb-5">
            ...
            <div className="mb-5">
              <label className="block font-medium mb-2" style={{ fontSize: '13px' }}>Type</label>
            ...
            <div className="mb-5">
              <label className="block font-medium mb-2" style={{ fontSize: '13px' }}>Priority</label>
              <div className="flex gap-2">
                {[
                  ...
                  <div key={p.key} onClick={() => updatePriority(p.key)}
                    className="flex-1 flex items-center justify-center gap-[6px] py-2 rounded-md cursor-pointer"
                    style={{ border: '1px solid', borderColor: activeTicket.priority === p.key ? '#fcd34d' : '#e2e8f0', background: activeTicket.priority === p.key ? '#fef3c7' : 'transparent', fontSize: '12px' }}>
              ...
              <div className="mb-5">
                ...
                <div className="flex gap-2 flex-wrap">
                  {activeTicket.tags.map((tag) => (
                    <div key={tag} className="px-[10px] py-[6px] rounded-[20px] text-white font-medium" style={{ background: '#0f172a', fontSize: '11px' }}>{tag}</div>
                  ))}
                </div>
              </div>
            )}
