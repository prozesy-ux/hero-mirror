...
const SellerOrders = () => {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { icon: AlertCircle, label: status, bg: 'bg-gray-100', text: 'text-gray-700', border: 'border-l-gray-400' };
    }
  };
  ...
  const statCards = [
    ...
    { title: 'Delivered', value: deliveredCount, icon: Truck, iconBg: 'bg-blue-100', iconColor: 'text-blue-600', valueColor: 'text-blue-600' },
    { title: 'Completed', value: completedCount, icon: CheckCircle, iconBg: 'bg-emerald-100', iconColor: 'text-emerald-600', valueColor: 'text-emerald-600' },
    { title: 'Refunded', value: refundedCount, icon: XCircle, iconBg: 'bg-red-100', iconColor: 'text-red-600', valueColor: 'text-red-600' },
  ];

  return (
    <div className="bg-[#f1f5f9] min-h-screen p-8 space-y-6 seller-dashboard" style={{ fontFamily: '"Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#1F2937]">Orders</h2>
          <p className="text-sm text-[#6B7280]">Manage and track your orders</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="min-w-[160px] justify-between bg-white border-gray-200 rounded-xl">
                <div className="flex items-center gap-2">
                  <CalendarIcon className="h-4 w-4 text-[#6B7280]" />
                  <span className="text-[#1F2937]">{dateLabels[dateRange]}</span>
                </div>
              ...
              <div className="space-y-1">
                {['today', 'yesterday', 'week', 'month', '30d', 'all'].map((key) => (
                  <Button
                    ...
                    className={`w-full justify-start rounded-xl ${dateRange === key ? 'bg-[#FF7F00]/10 text-[#FF7F00]' : ''}`}
                  >
                    {dateLabels[key]}
                  </Button>
                ))}
                <Button
                  ...
                  className={`w-full justify-start rounded-xl ${dateRange === 'custom' ? 'bg-[#FF7F00]/10 text-[#FF7F00]' : 'text-[#FF7F00]'}`}
                >
                  Custom Range...
                </Button>
              </div>
            ...
            <Button
              ...
              className={`flex-shrink-0 rounded-full px-4 ${
                activeStatus === tab.key 
                  ? 'bg-[#FF7F00] text-white hover:bg-[#e67200]' 
        ...
        {selectedOrders.size > 0 && (
          <div className="flex items-center gap-3 mt-4 pt-4 border-t border-gray-100">
            <Badge className="bg-blue-100 text-blue-700 rounded-full">{selectedOrders.size} selected</Badge>
            <Button size="sm" variant="ghost" onClick={() => setSelectedOrders(new Set())} className="text-[#6B7280]">
              Clear Selection
            </Button>
          </div>
        )}
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <h3 className="font-semibold text-[#1F2937] mb-2">No orders found</h3>
          ...
          filteredOrders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                {/* Order Header */}
                <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <div className="flex items-center gap-2 text-sm text-[#6B7280] flex-wrap">
                    <span>Customer: <strong className="text-[#1F2937]">{order.buyer?.email?.split('@')[0] || 'Buyer'}</strong></span>
                    <span className="text-gray-300 hidden sm:inline">|</span>
                    <span>Date: <strong className="text-[#1F2937]">{format(new Date(order.created_at), 'dd MMM, yyyy')}</strong></span>
                  </div>
                ...
                <div className="p-6 flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-[#1F2937] truncate">{order.product?.name || 'Product'}</p>
                    <p className="text-sm text-[#6B7280]">Quantity: 1</p>
                    {buyerEmailInput && (
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs text-[#6B7280]">{buyerEmailInput}</span>
                        <Button size="sm" variant="ghost" onClick={() => copyToClipboard(buyerEmailInput)} className="h-5 w-5 p-0 text-[#FF7F00]">
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="font-bold text-lg text-[#1F2937]">{formatAmountOnly(Number(order.amount))}</p>
                    <p className="text-xs text-emerald-600">+{formatAmountOnly(Number(order.seller_earning))} earned</p>
                  </div>
                  
                  <Badge className={`${statusConfig.bg} ${statusConfig.text} border-0 text-xs font-semibold rounded-full px-3 py-1 flex-shrink-0`}>
                  ...
                  <div className="flex gap-2 w-full sm:w-auto flex-shrink-0">
                    {order.status === 'pending' && (
                      (() => {
                        ...
                        if (isAutoDelivery) {
                          return (
                            <div className="flex items-center gap-2">
                              <Badge className="bg-emerald-100 text-emerald-700 border-0 text-xs font-semibold rounded-full">
                              ...
                              <Button
                                ...
                                className="text-xs text-[#6B7280] hover:text-[#1F2937] h-7 px-2"
                        ...
                        return (
                          <Button
                            size="sm"
                            onClick={() => setSelectedOrder(order.id)}
                            className="flex-1 sm:flex-none bg-[#FF7F00] hover:bg-[#e67200] text-white rounded-xl"
                          >
                    ...
                    {order.status === 'delivered' && (
                      <Button
                        ...
                        className="flex-1 sm:flex-none border-emerald-500 text-emerald-600 rounded-xl hover:bg-emerald-50"
                      >
                    ...
                    {order.status === 'completed' && (
                      <Button
                        ...
                        className="flex-1 sm:flex-none text-[#6B7280] rounded-xl"
                      >
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                className="flex-1 bg-[#FF7F00] hover:bg-[#e67200] text-white rounded-xl"
              >
                {delivering ? (
